...
var Response = function() {
	 ...
	 * otherwise. If an @param->errorHandler is passed, it is called instead.
	 *
	 * Note: an error handler function will receive the context of the current response object as its context and
	 * the current http-response as its first parameter. It then becomes YOUR responsibility to write to the response
	 * socket and close it.
	 */
	this.respondWithFile = function(response, pathToFile, fileMimeType, errorHandler) {
		...
		fs.readFile(Globals.rootDirectory + '/' + pathToFile, function(error, data) {
			...
			if(error) {
				...
				// if an error handler is defined, use that, else handle error
				if(errorHandler && typeof errorHandler == 'function') {
					return errorHandler.call(scope, response);
				} else {
	...
	/**
	 * Serves context of the main application index file.
	 * Values of this request are set in Globals.js
	 */
	this.respondWithIndex = function(response) {
		this.respondWithFile(response, '/index.html', 'text/html');
	}
