...
Handlers.getCurrentRequest = function() {
...
Handlers.handleRequestAsFileStream = function(request, response) {
	...
	Responses.respondWithFile(response, pathToFile, fileMimeType, function(response) {
		this.respondWithIndex(response);
	});
};
...
Handlers.mainRequestHandler = function(request, response) {
	currentRequest = Routers.requestRouter(request, response);

	if (typeof currentRequest == 'function') {
		Handlers[currentRequest()](request, response);
	} else if (currentRequest.match(/^\/api\/([a-z0-9\/])+/gi)) {
